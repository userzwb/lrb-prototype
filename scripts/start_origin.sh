...
rm -f /tmp/influx.log
touch /tmp/influx.log ; tail -f /tmp/influx.log | while read v; do curl -m 1 -XPOST 'http://mmx.cs.princeton.edu:8086/write?db=mydb' -u admin:system --data-binary "$v";done &>/dev/null &
