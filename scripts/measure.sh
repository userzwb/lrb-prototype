...
echo "downloading..."
scp -3 -o ProxyJump=${proxy_ip_external} "$origin_ip_internal":~/webtracereplay/log/* ./
scp -3 -o ProxyJump=${proxy_ip_external} "$client_ip_internal":~/webtracereplay/log/* ./
rsync "$proxy_ip_external":~/webtracereplay/log/* ./
scp -3 "$proxy_ip_external":/opt/ts/var/log/trafficserver/diag.log ./
